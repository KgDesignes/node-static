...
this.Server.prototype.servePath = function (pathname, status, headers, req, res, finish) {
    ...
    var slashEnded = ( pathname.lastIndexOf('/') == (pathname.length - 1) );
        ...
        fs.stat(pathname, function (e, stat) {
            if (e) {
                ...
                if ( ! slashEnded ) {
                    ...
                    console.log('Appending slash %j', parsedUrl);

                    parsedUrl.pathname += '/';

                    finish(301, { 'Location': url.format(parsedUrl) });
                } else {
...
this.Server.prototype.respondNoGzip = function (pathname, status, contentType, _headers, files, stat, req, res, finish) {
    ...
    // only set if caching is enabled
    if(this.cache !== false) {
    ...
    if (this.cache !== false && (req.headers['if-none-match'] === headers['ETag'] ||
